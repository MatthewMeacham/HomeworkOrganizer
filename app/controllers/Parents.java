...
public class Parents extends Controller {
	...
	public Result redirectToStudent(Long studentID, String parentID) {
		...
		return badRequest(parentProfile.render(parent, children,
				Utilities.createAssignmentsListForParent(parent),
				Utilities.today, "", ""));
	}
	...
	public Result addChild(String parentID) {
		...
		if (filledForm.hasErrors()) {
			Parent parent = Parent.find.ref(Long.valueOf(parentID));
			return badRequest(parentProfile.render(parent,
					Utilities.createChildrenList(parent),
					Utilities.createAssignmentsListForParent(parent),
					Utilities.today, "addChild", "Error while processing."));
		} else {
			Parent parent = Parent.find.ref(Long.valueOf(parentID));
			...
			if (Integer.valueOf(grade) <= MIN_GRADE
				...
				return badRequest(parentProfile.render(parent,
						Utilities.createChildrenList(parent),
						Utilities.createAssignmentsListForParent(parent),
						Utilities.today, "addChild", "Invalid grade level."));

			if (name.equals("") || name.trim().isEmpty())
				return badRequest(parentProfile.render(parent,
						Utilities.createChildrenList(parent),
						Utilities.createAssignmentsListForParent(parent),
						Utilities.today, "addChild", "Invalid name."));
			if (name.length() >= 250)
				return badRequest(parentProfile.render(parent,
						Utilities.createChildrenList(parent),
						Utilities.createAssignmentsListForParent(parent),
						Utilities.today, "addChild", "Name was too long."));

			if (password.equals("") || password.trim().isEmpty()
					|| password.equals(parent.password))
				return badRequest(parentProfile.render(parent,
						Utilities.createChildrenList(parent),
						Utilities.createAssignmentsListForParent(parent),
						Utilities.today, "addChild", "Invalid password"));
			...
			Student newStudent = Student.create(name,
					filledForm.data().get("email").toLowerCase(), parent.salt,
					password, grade);
			if (newStudent == null) {
				return badRequest(parentProfile
						.render(parent,
								Utilities.createChildrenList(parent),
								Utilities
										.createAssignmentsListForParent(parent),
								Utilities.today, "addChild",
								"Can't have the same password as another one of your kids."));
			}
			return ok(parentProfile.render(parent,
					Utilities.createChildrenList(parent),
					Utilities.createAssignmentsListForParent(parent),
	...
	public Result changeParentAccountSettings(String parentID, String studentID) {
		...
		try {
			currentPassword = HASHER.hashWithSaltSHA256(
					filledForm.data().get("currentPassword"), parent.salt);
			newPassword = HASHER.hashWithSaltSHA256(
					filledForm.data().get("newPassword"), parent.salt);
			newPasswordAgain = HASHER.hashWithSaltSHA256(
					filledForm.data().get("newPasswordAgain"), parent.salt);
		...
		if (studentID.equals("0")) {
			...
			if (!name.equals(parent.name)) {
				if (name.equals("") || name.trim().isEmpty())
					return badRequest(parentProfile
							.render(parent, children, Utilities
									.createAssignmentsListForParent(parent),
									Utilities.today, "accountSettings",
									"Invalid name."));
				if (name.length() >= 250)
						...
						return badRequest(parentProfile
								.render(parent,
										...
										Utilities
												.createAssignmentsListForParent(parent),
										Utilities.today, "accountSettings",
										"One of your children already has that name."));
				}
				parent.name = name;
				parent.save();
			}
			...
			if (!currentPassword.equals("") && !newPassword.equals("")
				...
				if (currentPassword.equals("")
						|| !currentPassword.equals(parent.password))
					return badRequest(parentProfile.render(parent, children,
							Utilities.createAssignmentsListForParent(parent),
							Utilities.today, "accountSettings",
				...
				for (Student child : children) {
					if (child.password.equals(newPassword))
						return badRequest(parentProfile
								.render(parent,
										children,
										Utilities
												.createAssignmentsListForParent(parent),
										Utilities.today, "accountSettings",
										"Child can not have the same password as the parent."));
				}
				parent.password = newPassword;
				parent.save();

			}
			return ok(parentProfile.render(parent, children,
					Utilities.createAssignmentsListForParent(parent),
					Utilities.today, "overview",
			...
			if (!name.equals(student.name)) {
				if (name.equals("") || name.trim().isEmpty())
					return badRequest(parentProfile
							.render(parent, Utilities
									.createChildrenList(parent), Utilities
									.createAssignmentsListForParent(parent),
									Utilities.today, "accountSettings",
									"Invalid name."));
				if (name.length() >= 250)
			...
			if (!currentPassword.equals("") && !newPassword.equals("")
				...
				if (!newPassword.equals(newPasswordAgain))
					return badRequest(parentProfile.render(parent,
							...
							Utilities.today, "accountSettings",
							"New passwords did not match."));
				if (newPassword.equals(parent.password))
					return badRequest(parentProfile
							.render(parent, Utilities
									.createChildrenList(parent), Utilities
									.createAssignmentsListForParent(parent),
				...
				for (Student child : children) {
					if (child == student)
						continue;
					if (newPassword.equals(child.password))
						return badRequest(parentProfile
								.render(parent,
										children,
										Utilities
												.createAssignmentsListForParent(parent),
										Utilities.today, "accountSettings",
										"Child can not have the same password as another."));
				}
